    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white p-6 flex justify-center items-center">
        <form onSubmit={handleVerifyKey} className="w-full max-w-sm bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-4 shadow-2xl text-center">
          ...
          <p className="text-xs text-slate-400">Enter master security key.</p>
          ...
          <input
            ...
            className="w-full p-3 bg-slate-800 rounded-xl border border-slate-700 text-white text-sm text-center focus:outline-none"
          />
          ...
          <form onSubmit={handleCreateCustomer} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full p-3 bg-slate-800 rounded-xl border border-slate-700 mt-1 text-white text-sm focus:outline-none"
              />
            </div>
            ...
            <div>
              <label className="text-xs text-slate-400">Set Password (8+ characters)</label>
              <input
                type="text"
                required
                minLength={8}
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-4 shadow-xl flex flex-col justify-between">
          <div>
            ...
            <div className="mt-4 space-y-3 max-h-[380px] overflow-y-auto pr-1">
              {loadingUsers ? (
                <p className="text-xs text-slate-400 text-center py-4">Loading list...</p>
              ) : userList.length === 0 ? (
                ...
                userList.map((usr) => (
                  <div
                    ...
                    className={`p-3 rounded-xl border transition text-xs flex flex-col gap-2 ${
                      selectedUserId === usr.id ? 'bg-slate-800 border-blue-500' : 'bg-slate-950 border-slate-800'
                    }`}
                  >
                    ...
                    {selectedUserId === usr.id && (
                      <div className="mt-2 pt-2 border-t border-slate-800 space-y-2">
                        <input
                          ...
                          className="w-full p-2 bg-slate-900 rounded-lg border border-slate-700 text-white text-xs focus:outline-none"
                        />
                        <button
